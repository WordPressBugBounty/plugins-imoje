...
class WC_Gateway_Imoje_RestApi_Blocks extends AbstractPaymentMethodType
{
	...
	private function enqueue_imoje_blik_payment_script()
	{
		...
		$settings = get_option('woocommerce_imoje_blik_settings', []);

		$imojeBlik = new WC_Gateway_ImojeBlik();
		$payment_methods = $this->get_block_checkout_tooltip( $imojeBlik->get_payment_channels());
		...
		wp_localize_script(
			...
			[
				...
				'logo_blik_oneclick'   => '',
				'settings_blik'        => $settings,
				'blik_text'            => __('Enter the correct 6-digit BLIK code.', 'imoje'),
				'payment_methods_blik' => $payment_methods,
	...
	private function enqueue_imoje_payment_script()
	{
		...
		$settings = get_option('woocommerce_imoje_settings', []);
	...
	private function enqueue_imoje_paylater_payment_script()
	{
		...
		$settings = get_option('woocommerce_imoje_paylater_settings', []);
		$imojePaylater = new WC_Gateway_ImojePaylater();
		$payment_methods = $this->get_block_checkout_tooltip($imojePaylater->get_payment_channels());
		...
		wp_localize_script(
			...
			[
				...
				'settings_paylater'        => $settings,
				'payment_methods_paylater' => $payment_methods,
	...
	private function enqueue_imoje_cards_payment_script()
	{
		...
		$settings = get_option('woocommerce_imoje_cards_settings', []);
	...
	private function enqueue_imoje_pbl_payment_script()
	{
		...
		$settings = get_option('woocommerce_imoje_pbl_settings', []);
		$imojePbl = new WC_Gateway_ImojePbl;
		$payment_methods = $this->get_block_checkout_tooltip($imojePbl->get_payment_channels());
		...
		wp_localize_script(
			...
			[
				...
				'settings_pbl'        => $settings,
				'payment_methods_pbl' => $payment_methods,
			]
		);
	}
	...
	private function enqueue_imoje_visa_payment_script()
	{
		...
		$settings = get_option('woocommerce_imoje_visa_settings', []);
	...
	private function enqueue_imoje_installments_payment_script()
	{
		...
		$settings = get_option('woocommerce_imoje_installments_settings', []);
		...
		wp_localize_script(
			...
			[
				...
				'settings_installments' => $settings,
