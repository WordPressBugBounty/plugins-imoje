( function () {
	...
	const data = wc.wcSettings.getSetting( 'imoje_pbl_data' );
	...
	const Component = ( props ) => {
		...
		const [ selectedMethodCode, setSelectedMethodCode ] = React.useState( null );
		...
		React.useEffect( () => {
			const unsubscribe = props.eventRegistration.onPaymentSetup( async ( processingProps ) => {
				...
				return {
					...
					meta: {
						paymentMethodData: {
							'imoje-selected-channel': `${ selectedMethodCode }-${ selectedMethod }`,
						},
					},
				};
			} );
		...
		return React.createElement(
			...
			paymentChannelsBlock(data.payment_methods, selectedMethod, setSelectedMethod, tooltip, setTooltip, setSelectedMethodCode),
